...
void timeval_from_clock_ticks(clock_t ticks, timeval *tv) {
	tv->tv_sec = ticks / abi::time::clock_ticks_per_second;
	tv->tv_usec = (ticks % abi::time::clock_ticks_per_second)
			* abi::time::microseconds_per_second
			/ abi::time::clock_ticks_per_second;
}
...
int map_itimer_clock(int which, int *clock) {
	switch(which) {
		case ITIMER_REAL:
			*clock = abi::hrtsys::clock::standard;
			return 0;
		case ITIMER_VIRTUAL:
			*clock = abi::hrtsys::clock::user_virtual;
			return 0;
		case ITIMER_PROF:
			*clock = abi::hrtsys::clock::process_virtual;
			return 0;
		default:
			return EINVAL;
	}
...
int validate_timeval_value(const struct timeval &tv) {
	if(tv.tv_sec < 0 || tv.tv_usec < 0 || tv.tv_usec >= abi::time::microseconds_per_second)
		return EINVAL;
	return 0;
}
...
svr4_hrtime make_svr4_hrtime(const struct timeval &tv) {
	return svr4_hrtime{
		...
		static_cast<unsigned long>(abi::time::microseconds_per_second)
	};
}

int convert_svr4_hrtime(const svr4_hrtime &time, struct timeval *tv) {
	if(time.res != static_cast<unsigned long>(abi::time::microseconds_per_second))
		return EIO;
	if(time.rem < 0 || time.rem >= abi::time::microseconds_per_second)
		return EIO;
...
int run_hrt_alarm_command(svr4_hrtcmd *command) {
	if(int e = syscall_call(SYS_hrtsys, abi::hrtsys::opcode::alarm, command, 1).error(); e)
		return e;
	if(command->flags & abi::hrtsys::flag::error)
		return command->error ? command->error : EIO;
	if(command->flags & abi::hrtsys::flag::done)
		return 0;
	return EIO;
}
...
int read_itimer_locked(int which, struct itimerval *curr_value) {
	...
	svr4_hrtcmd command{};
	command.cmd = abi::hrtsys::command::bsd_pend;
	command.clk = clock;
...
int write_itimer_locked(int which, const struct itimerval *new_value) {
	...
	if(timeval_is_zero(new_value->it_value)) {
		svr4_hrtcmd cancel{};
		cancel.cmd = abi::hrtsys::command::bsd_cancel;
		cancel.clk = clock;
	...
	if(timeval_is_zero(new_value->it_interval)) {
		command.cmd = abi::hrtsys::command::bsd;
		command.interval = make_svr4_hrtime(new_value->it_value);
	} else {
		command.cmd = abi::hrtsys::command::bsd_rep;
...
int translate_hrtime_result(const svr4_hrtime &time, time_t *secs, long *nanos) {
	if(time.res != abi::time::nanoseconds_per_second)
		return EIO;
	if(time.rem < 0 || static_cast<unsigned long>(time.rem) >= abi::time::nanoseconds_per_second)
		return EIO;
...
int fetch_realtime(time_t *secs, long *nanos) {
	svr4_hrtime time{0, 0, abi::time::nanoseconds_per_second};
	if(int e = syscall_call(SYS_hrtsys, abi::hrtsys::opcode::cntl,
			abi::hrtsys::command::tofd, abi::hrtsys::clock::standard,
			static_cast<svr4_interval *>(nullptr), &time).error(); e)
		return e;
...
int fetch_monotonic(time_t *secs, long *nanos) {
	...
	while(state != 2) {
		if(state == 0) {
			...
			if(__atomic_compare_exchange_n(&init_state, &expected, 1, false,
				...
				if(int e = syscall_call(SYS_hrtsys, abi::hrtsys::opcode::cntl,
						abi::hrtsys::command::start_it,
						abi::hrtsys::clock::standard, &start_interval,
	...
	svr4_hrtime time{0, 0, abi::time::nanoseconds_per_second};
	if(int e = syscall_call(SYS_hrtsys, abi::hrtsys::opcode::cntl,
			abi::hrtsys::command::get_it, abi::hrtsys::clock::standard,
			&start_interval, &time).error(); e)
		return e;
	return translate_hrtime_result(time, secs, nanos);
}
...
unsigned int allocate_tls_selector() {
	static unsigned int next_selector_index = abi::tls::first_selector_index;
	auto index = __atomic_fetch_add(&next_selector_index, 1U, __ATOMIC_RELAXED);
	if(index >= abi::tls::max_ldt_index)
		return 0;
...
constexpr rlim_t translate_from_kernel_rlimit(rlim_t value) {
	return value == abi::rlimit::infinity ? RLIM_INFINITY : value;
}

constexpr rlim_t translate_to_kernel_rlimit(rlim_t value) {
	return value == RLIM_INFINITY ? abi::rlimit::infinity : value;
}
...
void Sysdeps<LibcLog>::operator()(const char *message) {
	...
	(void)syscall_call(SYS_clocal, abi::clocal::debugcon_write, message, length, 0, 0);
}
...
int Sysdeps<GetHostname>::operator()(char *buffer, size_t bufsize) {
	...
	if(int e = syscall_call(SYS_systeminfo, abi::systeminfo::hostname, buffer, bufsize)
...
int Sysdeps<Umask>::operator()(mode_t mode, mode_t *old) {
    return syscall_call(SYS_umask, mode).store(old);
}
...
int Sysdeps<VmMap>::operator()(void *hint, size_t size, int prot, int flags,
	...
	void *mapped_window;
	int ret_error = syscall_call(SYS_mmap, hint, size, prot, flags, fd, offset).store(&mapped_window);
	if(ret_error) {
...
int Sysdeps<Sigpending>::operator()(sigset_t *set) {
	return syscall_call(SYS_sigpending, abi::sigpending::query, set).error();
}
...
int Sysdeps<Waitpid>::operator()(pid_t pid, int *status, int flags, struct rusage *ru, pid_t *ret_pid) {
	...
	if(pid > 0) {
		...
		kernel_idtype = 2;
		if(int e = syscall_call(SYS_pgrpsys, abi::pgrpsys::get_pgid, 0).store(&id); e)
			return e;
	} else {
		kernel_idtype = 2;
		id = -pid;
...
int Sysdeps<TcbSet>::operator()(void *pointer) {
	...
	ssd request = {
		...
		.ls = 0xFFFFF,
		.acc1 = abi::tls::user_data_acc1,
		.acc2 = abi::tls::data_acc2,
	};

	if(int e = syscall_call(SYS_sys3b, abi::sysi86::dscr, &request, 0, 0).error(); e)
		return e;
...
pid_t Sysdeps<FutexTid>::operator()() {
	pid_t tid;
	if(int e = syscall_call(SYS_thread_self).store(&tid))
		return -e;
	return tid;
}

int Sysdeps<FutexWait>::operator()(int *pointer, int expected, const struct timespec *time) {
	return syscall_call(SYS_futex_wait, pointer, expected, time).error();
}

int Sysdeps<FutexWake>::operator()(int *pointer, bool all) {
	return syscall_call(SYS_futex_wake, pointer, all ? 1 : 0).error();
}

int Sysdeps<GetPgid>::operator()(pid_t pid, pid_t *pgid) {
	return syscall_call(SYS_pgrpsys, abi::pgrpsys::get_pgid, pid).store(pgid);
}

int Sysdeps<GetSid>::operator()(pid_t pid, pid_t *sid) {
	return syscall_call(SYS_pgrpsys, abi::pgrpsys::get_sid, pid).store(sid);
}

int Sysdeps<SetPgid>::operator()(pid_t pid, pid_t pgid) {
	return syscall_call(SYS_pgrpsys, abi::pgrpsys::set_pgid, pid, pgid).error();
}

int Sysdeps<SetSid>::operator()(pid_t *sid) {
	return syscall_call(SYS_pgrpsys, abi::pgrpsys::set_sid).store(sid);
}
...
int Sysdeps<Isastream>::operator()(int fd) {
	int bytes_available = 0;
	auto result = syscall_call(SYS_ioctl, fd, I_NREAD, &bytes_available);
	if(!result.failed)
		return 1;
	if(result.value == ENOTTY || result.value == EINVAL || result.value == ENOSTR)
		return 0;
	return result.value;
}
...
int Sysdeps<Sleep>::operator()(time_t *secs, long *nanos) {
	...
	if(*secs < 0 || *nanos < 0
			|| static_cast<unsigned long>(*nanos) >= abi::time::nanoseconds_per_second)
		return EINVAL;

	svr4_hrtcmd command{};
	command.cmd = abi::hrtsys::command::int_sleep;
	command.clk = abi::hrtsys::clock::standard;
	command.interval = {
		static_cast<unsigned long>(*secs),
		*nanos,
		abi::time::nanoseconds_per_second
	};

	auto ret = syscall_call(SYS_hrtsys, abi::hrtsys::opcode::sleep, &command);
	if(int e = ret.error(); e) {
		if(e != EINTR)
			return e;
		return translate_hrtime_result(command.interval, secs, nanos);
	}
...
int Sysdeps<Semget>::operator()(key_t key, int n, int fl, int *id) {
	return syscall_call_always_restart(SYS_semsys, abi::semsys::semget, key, n, fl).store(id);
}

int Sysdeps<Semctl>::operator()(int semid, int semnum, int cmd, void *semun, int *ret_value) {
	return syscall_call_always_restart(SYS_semsys, abi::semsys::semctl,
			semid, semnum, cmd, semun).store(ret_value);
}

int Sysdeps<Semop>::operator()(int semid, struct sembuf *sops, size_t nsops) {
	return syscall_call(SYS_semsys, abi::semsys::semop, semid, sops, nsops).error();
}

int Sysdeps<Shmat>::operator()(void **seg_start, int shmid, const void *shmaddr, int shmflg) {
	return syscall_call_always_restart(SYS_shmsys, abi::shmsys::shmat,
			shmid, shmaddr, shmflg).store(seg_start);
}

int Sysdeps<Shmctl>::operator()(int *idx, int shmid, int cmd, struct shmid_ds *buf) {
	return syscall_call_always_restart(SYS_shmsys, abi::shmsys::shmctl,
			shmid, cmd, buf).store(idx);
}

int Sysdeps<Shmdt>::operator()(const void *shmaddr) {
	return syscall_call_always_restart(SYS_shmsys, abi::shmsys::shmdt, shmaddr).error();
}

int Sysdeps<Shmget>::operator()(int *shm_id, key_t key, size_t size, int shmflg) {
	return syscall_call_always_restart(SYS_shmsys, abi::shmsys::shmget,
			key, size, shmflg).store(shm_id);
}

int Sysdeps<Msgctl>::operator()(int q, int cmd, struct msqid_ds *buf) {
	return syscall_call_always_restart(SYS_msgsys, abi::msgsys::msgctl, q, cmd, buf).error();
}

int Sysdeps<Msgget>::operator()(key_t k, int flag, int *out) {
	return syscall_call_always_restart(SYS_msgsys, abi::msgsys::msgget, k, flag).store(out);
}

int Sysdeps<Msgrcv>::operator()(int msqid, void *msgp, size_t msgsz, long msgtyp, int msgflg, ssize_t *out) {
	return syscall_call(SYS_msgsys, abi::msgsys::msgrcv,
			msqid, msgp, msgsz, msgtyp, msgflg).store(out);
}
